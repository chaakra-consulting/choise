...
<div class="col-md-5 col-sm-12">
    ...
    ?>
        <div class="col-sm-2 justify-content-center text-center" <?php
                                                                    $query = $this->db->query("SELECT * FROM tb_data_jawaban_learning_agility WHERE id_lowongan = $id_lowongan AND no_soal = $i AND id_ujian = 1 AND id_pelamar = $id_pelamar");
                                                                    if ($query->num_rows() > 0) {
                                                                        $warna = '#8ad919';
                                                                        $warnaText = '#fff';
                                                                    } else {
            ...
            <a href="<?php echo base_url('Pelamar/Ujian/frame_ujian_learning_agility/1/' . $i) ?>" style="color: <?php echo $warnaText ?>"><b><?= $i; ?></b></a>
        </div>
    <?php } ?>
</div>
