...
<div class="col-sm-12 main">
    <div class="row" style="margin-bottom: 10px; z-index: 1; display: flex; align-items: center;">
        <div class="col-xs-6 col-lg-6">
            <h3 class="exam-header-title">TIKI Sub 2</h3>
        </div>
    ...
    $(document).ready(function() {
        ...
        window.loadQuestion = function(nomor) {
           ...
           $("#question-grid .selected").removeClass('selected');
            ...
            $.ajax({
                ...
                type: 'POST',
                dataType: 'json',
                data: { id_ujian: id_ujian, nomor_soal: currentSoal, subtes: 2 },
                success: function(res) {
                    $('#display-pertanyaan').html(res.soal ? `<img src="<?=  base_url('upload/bank_soal/tiki_d/subtes2/'); ?>soal-${currentSoal}/soal.png" style="max-width:100%">` : res.soal);

                    $('#opsi-a').attr('src', res.opsi_a ? `<?=  base_url('upload/bank_soal/tiki_d/subtes2/'); ?>soal-${currentSoal}/a.png` : '');
                    $('#opsi-b').attr('src', res.opsi_b ? `<?=  base_url('upload/bank_soal/tiki_d/subtes2/'); ?>soal-${currentSoal}/b.png` : '');
                    $('#opsi-c').attr('src', res.opsi_c ? `<?=  base_url('upload/bank_soal/tiki_d/subtes2/'); ?>soal-${currentSoal}/c.png` : '');
                    $('#opsi-d').attr('src', res.opsi_d ? `<?=  base_url('upload/bank_soal/tiki_d/subtes2/'); ?>soal-${currentSoal}/d.png` : '');
                    $('#opsi-e').attr('src', res.opsi_e ? `<?=  base_url('upload/bank_soal/tiki_d/subtes2/'); ?>soal-${currentSoal}/e.png` : '');
                    $('#opsi-f').attr('src', res.opsi_f ? `<?=  base_url('upload/bank_soal/tiki_d/subtes2/'); ?>soal-${currentSoal}/f.png` : '');
        ...
        $('#btn-next').click(function() {
            if (currentSoal < totalSoal) {
                ...
                if (konfirmasi) {
                    window.location.href = '<?php echo base_url("Pelamar/Daftar_ujian/Tiki_d/latihan3"); ?>';
                }
            }
        });
