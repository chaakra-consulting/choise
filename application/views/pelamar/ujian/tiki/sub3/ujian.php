...
<style>
    ...
    .custom-radio-card {
        ...
        min-width: 60px;
        /* Slightly wider to accommodate images */
    }
    ...
    .custom-radio-card .card-content {
        ...
        /* Stack letter on top, image on bottom */
        align-items: center;
        justify-content: center;
        height: 100%;
        overflow: hidden;
        /* Contains the scaled image inside rounded corners */
    }
    ...
    .option-image {
        max-width: 100%;
        height: 55px;
        /* Restrict height so the grid stays uniform */
        object-fit: contain;
    ...
    .question-image-wrapper img {
        ...
        border-radius: 4px;
        mix-blend-mode: multiply;
        /* Blends the question image nicely too */
    }
...
<div class="col-sm-12 main">
    <div class="row" style="margin-bottom: 10px; z-index: 1; display: flex; align-items: center;">
        <div class="col-xs-6 col-lg-6">
            <h3 class="exam-header-title">TIKI Sub 3</h3>
        </div>
    ...
    $(document).ready(function() {
        ...
        window.loadQuestion = function(nomor) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    ...
                    $('#opsi-a').attr('src', res.opsi_a ? `<?= base_url('upload/bank_soal/tiki_d/subtes3/'); ?>soal-${currentSoal}/a.png` : '');
                    $('#opsi-b').attr('src', res.opsi_b ? `<?= base_url('upload/bank_soal/tiki_d/subtes3/'); ?>soal-${currentSoal}/b.png` : '');
                    $('#opsi-c').attr('src', res.opsi_c ? `<?= base_url('upload/bank_soal/tiki_d/subtes3/'); ?>soal-${currentSoal}/c.png` : '');
                    $('#opsi-d').attr('src', res.opsi_d ? `<?= base_url('upload/bank_soal/tiki_d/subtes3/'); ?>soal-${currentSoal}/d.png` : '');
                    $('#opsi-e').attr('src', res.opsi_e ? `<?= base_url('upload/bank_soal/tiki_d/subtes3/'); ?>soal-${currentSoal}/e.png` : '');
        ...
        $('#btn-next').click(function() {
            if (currentSoal < totalSoal) {
                ...
                if (konfirmasi) {
                    window.location.href = '<?php echo base_url("Pelamar/Daftar_ujian/Tiki_d/latihan4"); ?>';
                }
            }
        });
