...
<div class="col-sm-12 main">
    <div class="row" style="margin-bottom: 10px; z-index: 1; display: flex; align-items: center;">
        <div class="col-xs-6 col-lg-6">
            <h3 class="exam-header-title">Papikostik</h3>
        </div>
    ...
    <?php
    $id_ujian =  1;
    $ujian =  $this->db->query("SELECT * FROM tb_ujian_papi WHERE id_ujian_papi = 1");
                ...
                <form id="form-ujian" method="post">
                    ...
                    <div class="options-container">
                        <div class="custom-radio-card">
                            <input type="radio" id="aspek" name="jawaban" value="A">
                            <label for="aspek" class="card-content">
                                <span class="choice-letter">A. </span>
                                <span class="choice-text" id="pernyataan1"></span>
                            </label>
                        </div>
                        <div class="custom-radio-card">
                            <input type="radio" id="aspek2" name="jawaban" value="B">
                            <label for="aspek2" class="card-content">
                                <span class="choice-letter">B. </span>
                                <span class="choice-text" id="pernyataan2"></span>
                            </label>
                        </div>
...
<script type="text/javascript">
    $(document).ready(function() {
        ...
        function loadGrid(callback = null) {
            $.ajax({
                url: '<?php echo base_url("Pelamar/Daftar_ujian/Papikostik/get_grid"); ?>',
                type: 'POST',
                dataType: 'json',
                data: {
                    id_ujian: id_ujian,
        ...
        window.loadQuestion = function(nomor) {
            ...
            $('#pernyataan1, #pernyataan2').html(skeletonOpsi);
            // --- SKELETON LOADING ---
            ...
            // Fetch Data via AJAX
            $.ajax({
                url: '<?php echo base_url("Pelamar/Daftar_ujian/Papikostik/get_question"); ?>',
                type: 'POST',
                dataType: 'json',
                data: {
                    id_ujian: id_ujian,
                ...
                success: function(res) {
                    ...
                    $('#pernyataan1').html(res.pernyataan1);
                    $('#pernyataan2').html(res.pernyataan2);
                    $('#aspek').val(res.aspek);
                    $('#aspek2').val(res.aspek2);
        ...
        $('input[name="jawaban"]').on('change', function() {
            ...
            $.ajax({
                url: '<?php echo base_url("Pelamar/Daftar_ujian/Papikostik/save_answer"); ?>',
                type: 'POST',
                dataType: 'json',
                data: {
                    id_ujian: id_ujian,
