...
<div class="col-md-5 col-sm-12">
	...
	?>
		<div class="col-sm-1 justify-content-center text-center" <?php
																	$query = $this->db->query("SELECT * FROM $dbjawaban WHERE id_lowongan = $id_lowongan AND nomor_soal = $i AND id_ujian = $id_ujian AND id_pelamar = $id_pelamar AND seksi=3");
																	if ($query->num_rows() > 0) {
																		$warna = '#8ad919';
																		$warnaText = '#fff';
																	} else {
			...
			<a href="<?php echo base_url('Pelamar/Ujian/frame_ujian_tpa2_3/' . $id_ujian . '/' . $i) ?>" style="color: <?php echo $warnaText ?>"><b><?= $i; ?></b></a>
		</div>
	<?php } ?>
</div>
