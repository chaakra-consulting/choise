...
$idRmib = $this->session->userdata('ses_ujianRmib');
$idLowongan = $this->session->userdata('sesIdLowongan');

$rmib = $this->db->query("SELECT * FROM tb_ujian_rmib_wanita WHERE id_ujian_rmib_wanita = $idRmib");
foreach ($rmib->result() as $key_uRmib) {
	$timeRmib = $key_uRmib->waktu_akhir;
}
?>
...
<div class="col-sm-12 main">
   ...
   pekerjaan yang paling tidak di sukai
	...
	<div class="col-sm-12" style="background-color: #fff; padding: 30px; border-radius: 5px;">
		<div class="form-row">
			<form action="<?php echo base_url("Pelamar/Ujian/jawaban_rmib_wanita/") ?>" method="post">
				...
				<!-- <br>
				<label>1. ( Lima ) Kelebihan saya adalah :</label>
				</br>
					<textarea class="form-control" name="jawaban1" required=required></textarea>
				<br>	
					<label>2. ( Lima ) Kekurangan saya adalah :</label>
				</br>
