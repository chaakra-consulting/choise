...
<div class="col-sm-12 box">
		...
		<form method="post">
			...
			<center>
				...
				<div class="baten">
					<?php
					if ($jenis_tpa != 'aktif') {
						$jumlahsoal = $this->db->query("SELECT count(id_soal_tpa) as jumlah from $dbsoal where seksi=1 and jenis_tpa='pendek'")->result();
					} else {
						$jumlahsoal = $this->db->query("SELECT count(id_soal_tpa) as jumlah from $dbsoal where seksi=1")->result();
					}
					if ($array[0]->nomor_soal != 1) { ?>
						<button type="submit" style="margin-top: 5%" class="btn btn-primary" formaction="<?php echo base_url('Pelamar/Ujian/masukkan_jawaban_tpa1/1') ?>"> <i class="fa fa-arrow-circle-left"> </i> Sebelumnya
						</button>
					<?php } ?>
					...
					<?php if ($array[0]->nomor_soal != intval($jumlahsoal[0]->jumlah)) { ?>
						<button type="submit" style="margin-top: 5%" class="btn btn-primary" formaction="<?php echo base_url('Pelamar/Ujian/masukkan_jawaban_tpa1/2') ?>"> Selanjutnya <i class="fa fa-arrow-circle-right"></i>
						</button>
					<?php } ?>
					<?php if ($array[0]->nomor_soal >= intval($jumlahsoal[0]->jumlah)) { ?>
						<button type="submit" style="margin-top: 5%" class="btn btn-danger" formaction="<?php echo base_url('Pelamar/Ujian/masukkan_jawaban_endtpa1') ?>" class="btn btn-primary" onclick="return confirm('Apakah anda yakin ingin mengakhiri ujian?');"> Seksi Selanjutnya <i class="fa fa-arrow-circle-right"></i>
						</button>
					<?php } ?>
				</div>
			</center>
	...
	<?php
	$this->load->view('pelamar/ujian/tpa/panel_ujian_1');
	?>
