...
foreach ($array as $key) {
	...
	$kota_penempatan = $key['kota_penempatan'];
	$gaji = $key['gaji'];
	$gajia = $key['gaji'];
	...
	if ($id_motlet == 1) {
		$soal1 = "Jelaskan secara rinci mengenai impian dan target yang Anda miliki khususnya terkait dengan pencapaian karir dan kehidupan pribadi Anda pada JANGKA PENDEK (1 tahun kedepan) ";
		$soal2 = "Jelaskan secara rinci mengenai impian dan target yang Anda miliki khususnya terkait dengan pencapaian karir dan kehidupan pribadi Anda pada JANGKA MENENGAH (3 tahun mendatang)";
	} else {
		$soal1 = "Jelaskan secara rinci mengenai impian dan target yang Anda miliki khususnya terkait dengan pencapaian karir dan kehidupan pribadi Anda pada JANGKA PENDEK (1 tahun kedepan) ";
		$soal2 = "Jelaskan secara rinci mengenai impian dan target yang Anda miliki khususnya terkait dengan pencapaian karir dan kehidupan pribadi Anda pada JANGKA MENENGAH (3 tahun mendatang)";
	}
				...
				<form action="<?php echo base_url('Pelamar/Lamaran/apply') ?>" method="post">
					<div class="modal-body">
						<div class="form-group">
							...
							<label class="control-label">Pilih Kota Penempatan</label>
							<select name="kota_penempatan" class="form-control" required="required">
								<option value="">-- Pilih Kota --</option>
								<?php
								$kota_list = explode(',', $kota_penempatan);
								foreach ($kota_list as $kota) {
									if (!empty(trim($kota))) {
										echo '<option value="' . trim($kota) . '">' . trim($kota) . '</option>';
									}
								} ?>
							</select>
							<label class="control-label" style="margin-top: 3%"><?php echo $soal1 ?></label>
							...
							<textarea class="form-control" style="margin-bottom: 3%;" name="jawaban2" rows="3"><?php echo $jawaban2 ?></textarea>
							<?php if ($jenis_mot == 'Lowongan Kerja' || $jenis_mot == 'Kenaikan Jabatan') { ?>
								<label for="" class="label-group" style="margin: top 3%;">
									Berapa gaji yang anda inginkan? (* sebutkan nominalnya)
								</label>
								<input type="text" id="gaji_display" class="form-control" placeholder="Contoh: 5000000">
								<input type="hidden" id="gaji_hidden" name="gaji">
							<?php } else { ?>
								<input type="hidden" name="gaji" class="form-control" value="0">
							<?php } ?>
							<!-- KHUSU RSDU KRIAN -->
...
<script>
	document.addEventListener('DOMContentLoaded', function(){
		...
		const gajiHidden = document.getElementById('gaji_hidden');

		if(gajiDisplay){
			gajiDisplay.addEventListener('input', function(e){
				let rawValue = e.target.value.replace(/[^0-9]/g, '');

				gajiHidden.value = rawValue;
				if (rawValue) {
