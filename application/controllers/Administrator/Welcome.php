...
class Welcome extends CI_Controller
{
	...
	public function sync_login_sso()
	{
		...
		$getSsoData = $this->get_api('https://loginsso-chaakra.chaakra-consulting.com/api/AppController/getUserApp/' . $user_id . '/choise_2024');
		...
		$this->load->view('administrator/sync_login_sso', $data);
	}
	...
	public function sync_sso()
	{
		...
		$cek_user = $this->post_api('https://loginsso-chaakra.chaakra-consulting.com/api/UserController/cek_login', $data);
		$getRespon = json_decode($cek_user);

		if ($getRespon->success == false) {
			$this->session->set_flashdata('error', 'Akun sso tidak ada');
			redirect('Administrator/Welcome/sync_login_sso');
		} else {

			if ($this->session->userdata('ses_idLevel') != 'Administrator') {
				$this->session->set_flashdata('error', 'Sync sso hanya bisa dilakukan oleh administrator');
				redirect('Administrator/Welcome/sync_login_sso');
			}
			...
			$create_data = [
				...
				'app_key' => 'choise_2024',
				'user_app_id' => $this->session->userdata('ses_id'),
				'role' => 1,
				'redirect_url' => '-'
			];

			$createUserApp = $this->post_api('https://loginsso-chaakra.chaakra-consulting.com/api/AppController/createUserApp', $create_data);
