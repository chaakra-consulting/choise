...
class Data_nilai extends CI_Controller
{
	...
	{
		$benarcepat = 0;
		$datajcepat = $this->db->query("SELECT * FROM tb_data_jawaban_cepat WHERE id_pelamar=5276 AND id_lowongan =263")->result();
		$datasoalcepat = $this->db->query("SELECT * FROM tb_soal_cepat")->result();
		if (count($datacepat) != 0) {
			for ($i = 0; $i < count($datajcepat); $i++) {
				if ($datajcepat[$i]->jawaban == $datasoalcepat[$i]->jawaban) {
					$benarcepat = $benarcepat + 1;
				}
			}
		}

		$benaring = 0;
		$datajing = $this->db->query("SELECT * FROM tb_data_jawaban_inggris WHERE id_pelamar=5276 AND id_lowongan =263")->result();
		...
		$benaracc = 0;
		$datajacc = $this->db->query("SELECT * FROM tb_data_jawaban_tkb_accountingstaff WHERE id_pelamar=1 AND id_lowongan =263")->result();
		...
		$benarbisdev = 0;
		$datajbisdev = $this->db->query("SELECT * FROM tb_data_jawaban_tkb_bussinessdevelopmentstaff WHERE id_pelamar=1 AND id_lowongan =263")->result();
		...
		$benarproad = 0;
		$datajproad = $this->db->query("SELECT * FROM tb_data_jawaban_tkb_projectadministrationstaff WHERE id_pelamar=1 AND id_lowongan =263")->result();
		...
		$benartrain = 0;
		$datajtrain = $this->db->query("SELECT * FROM tb_data_jawaban_tkb_trainingoperationstaff WHERE id_pelamar=1 AND id_lowongan =263")->result();
		...
		$benarfront = 0;
		$datajfront = $this->db->query("SELECT * FROM tb_data_jawaban_tkb_frontlinerstaff WHERE id_pelamar=1 AND id_lowongan =263")->result();
		...
		echo "NILAI Bahasa	Cepat: " . $benarcepat * 1 . "<br>";	
