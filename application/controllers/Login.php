...
public function sso_login()
	{
		...
		if (isset($_POST['user_app_id'])) {
			...
			$getAdmin = $this->db->where('id_admin', $user_app_id)->get('tb_admin')->row_array();
			$level = $this->db->query("SELECT * FROM tb_level");
			foreach ($level->result() as $key_level) {
				if ($key_level->id_level == $getAdmin['id_level']) {
					$this->session->set_userdata('ses_idLevel', $key_level->nama_level);
				}
			}
			$this->session->set_userdata([
				'masuk'   => TRUE,
				'ses_nama' => $getAdmin['nama_admin'],
				'ses_id'  => $getAdmin['id_admin'],
			]);
			redirect('Administrator/Welcome');
		}

		// var_dump($user_app_id, $role);
		// exit;
		...
		if ($getRespon->success == true) {
			if ($getRespon->data_app != null) {
				$getAdmin = $this->db->where('id_admin', $getRespon->data_app->user_app_id)->get('tb_admin')->row_array();
				...
				$getRespon->redirect = base_url('Administrator/Welcome');
			}
		}
		sleep(1);
