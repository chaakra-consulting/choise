...
class Tes_paps_ugm extends CI_Controller
{
	...
	function index()
	{
		$this->load->view('pelamar/ujian/paps_ugm/index');
	}
	...
	public function get_all_data()
	{
		...
		$soal = $this->db->get('tb_soal_paps_ugm')->result();

		// 2. Fetch ALL answers the user has already saved
		$jawaban_query = $this->db->get_where('tb_data_jawaban_paps_ugm', [
			'id_ujian' => $id_ujian,
	...
	public function get_question()
	{
		...
		$soal = $this->db->get_where('tb_soal_paps_ugm', [
			'nomor_soal' => $nomor_soal,
			
		])->row();

		// Fetch user's previous answer if it exists
		$jawaban = $this->db->get_where('tb_data_jawaban_paps_ugm', [
			'nomor_soal' => $nomor_soal,
			'id_pelamar' => $id_pelamar,
			'id_lowongan' => $id_lowongan
		])->row();
	...
	public function get_grid()
	{
		...
		$total_soal = $this->db->count_all_results('tb_soal_paps_ugm');

		// Fetch only the question numbers that have been answered by this user
		$dijawab_query = $this->db->select('nomor_soal')->get_where('tb_data_jawaban_paps_ugm', [
			'id_ujian' => $id_ujian,
			'id_pelamar' => $id_pelamar,
			'id_lowongan' => $id_lowongan
		])->result_array();
	...
	public function save_answer()
	{
		...
		if ($jawaban) {
			...
			$cek = $this->db->get_where('tb_data_jawaban_paps_ugm', $where)->num_rows();

			if ($cek > 0) {
				$this->db->where($where)->update('tb_data_jawaban_paps_ugm', ['jawaban' => $jawaban]);
			} else {
				$data = $where;
				$data['jawaban'] = $jawaban;
				$this->db->insert('tb_data_jawaban_paps_ugm', $data);
			}
